...
export const MasonryProjectCard = ({ project, className = "" }: MasonryProjectCardProps) => {
  ...
  return (
    <Link href={`/projects/${project.id}`} className="block">
      <motion.div 
        className={`rounded-xl border border-neutral-300 dark:border-[#2E2E2E] bg-white dark:bg-[#111111] p-2 shadow-sm dark:shadow-none cursor-pointer h-full ${className}`}
        whileHover={{ 
          scale: 1.005,
      ...
      >
        <div className="flex flex-col gap-2 h-full">
          ...
          <div className="relative overflow-hidden rounded-lg h-48 sm:h-56">
            ...
            <div className="absolute bottom-0 w-full p-3 flex items-center justify-between bg-gradient-to-t from-black/80 via-black/40 to-transparent">
              <h3 className="font-normal text-white text-sm sm:text-base m-0 line-clamp-1">
                {project.title}
              </h3>
            </div>
          </div>
